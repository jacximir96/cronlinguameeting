...
$where_general = "typeConfig='urlSurvey'";
	...
	if($today_uni>=$last_week_time){
		...
		foreach ($course->coaches as $key => $coach) {
			...
			if(!empty($course->url_survey)){
				...
				$putUrl = $result_general[0]->valueConfig;
			}
			...
			$notifications->setId_type_not(7);
			$notifications->setData($data);
			$notifications->setDest_user($coach->id_coach);
...
function orderCourse($courses){
	...
	foreach ($courses as $session){
		...
		$newCourses[$id_course]->id_university = $session->id_university;
		$newCourses[$id_course]->name_university = $session->name_university;
		$newCourses[$id_course]->id_course = $session->course_id;
		$newCourses[$id_course]->name_course = $session->name_course;
		$newCourses[$id_course]->start_date = $session->date_ini_course;
		$newCourses[$id_course]->end_date = $session->date_end_course;
