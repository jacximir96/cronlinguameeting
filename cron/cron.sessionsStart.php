...
$select = "lm_students_courses.*,lm_students_courses_sessions_new.*,lm_courses_new.id_university,lm_time_zones.large_name,lm_users.name_user,"
        . "lm_users.lastname_user,lm_users.email,lm_users.emailsReception,lm_course_type.type_group";
...
if(count($result_students)>0){
    ...
    //echo "<pre>";echo print_r($new_students_flex);echo"</pre>";
	...
	if ($dateSelectIni->format('Y-m-d') == $tomorrow->format('Y-m-d') && $student->emailsReception == 1) {
	    ...
	    // send email
	    $subject = "LinguaMeeting session tomorrow";
	    $body = '<div>';
	    
	    $body .= "<br><br>You have a LinguaMeeting session scheduled for tomorrow! Your session is on <strong>" . $dateSelectIni->format('h:i A') . " (" . $student->large_name . ")</strong>.";
            ...
            $body .= '<br>'
                    ...
                    . '</ol>';
            $body .= '<a href="https://linguameeting.atlassian.net/servicedesk/customer/portal/1/article/15269914">Need to cancel or reschedule your session?</a> Do so at least 5 hours before!';
            $body .= '<br>This is a no-reply message. If you need to contact us, please send an email to <a href="mailto:support@linguameeting.com">support@linguameeting.com</a>.';
	    ...
	    if (empty($student->emailsReception)) {
